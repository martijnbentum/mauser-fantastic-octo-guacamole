# Description: This script processes the common voice French dataset.
...
directory = '../../LD/COMMON_VOICE_FRENCH/'
...
def make_files(force_make = False):
    if not force_make and Path('french_files.json').exists():
        with open('french_files.json', 'r') as f:
            files = json.load(f)
        return files
...
def make_force_align_pipeline(files = None):
    ...
    p = mauser.Pipeline(files, textgrid_directory, language = 'fra-FR')
    return p
