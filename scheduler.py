...
class every:
    ...
    def check_conditions(self):
        if len(self.conditions) > 2: 
            print('probably to many conditions',self.condtions)
        if (len(self.conditions) == 2 and 'weekday' not in self.conditions 
            and 'weekend' not in self.conditions): 
    ...
    def execute(self):
        e = threading.Thread(target= self.function,args=self.args,
            kwargs=self.kwargs)
        e.start()
...
def make_unit2seconds():
    day = 24*3600
    return {'second':1,'minute':60,'hour':3600,'day':day,'week':7*day,
...
def make_weekday_names2n():
    return {'Sunday':0,'Monday':1,'Tuesday':2,'Wednesday':3,'Thursday':4,
        'Friday':5,'Saturday':6}
