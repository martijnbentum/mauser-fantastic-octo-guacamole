...
directory = '../../LD/COMMON_VOICE_HUNGARIAN/'
...
def make_files(force_make = False):
    if not force_make and Path('hungarian_files.json').exists():
        with open('hungarian_files.json', 'r') as f:
            files = json.load(f)
        return files
    ...
    files = file_matcher.Files(audio_filenames, text_filenames, 
        'hungarian_files.json')
    return files.files

def make_force_align_pipeline(files = None):
    if not files: files = make_files()
    p = mauser.Pipeline(files, textgrid_directory, language = 'hun-HU')
    return p
