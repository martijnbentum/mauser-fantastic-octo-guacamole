...
directory = '../../LD/COMMON_VOICE_GERMAN/'
...
def make_files(force_make = False):
    if not force_make and Path('german_files.json').exists():
        with open('german_files.json', 'r') as f:
            files = json.load(f)
        return files
...
def make_force_align_pipeline(files = None):
    ...
    p = mauser.Pipeline(files, textgrid_directory, language = 'deu-DE')
    return p
