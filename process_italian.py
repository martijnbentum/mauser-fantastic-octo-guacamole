# Description: This script processes the common voice Italian dataset.
...
directory = '../../LD/COMMON_VOICE_ITALIAN/'
...
def make_files(force_make = False):
    if not force_make and Path('italian_files.json').exists():
        with open('italian_files.json', 'r') as f:
            files = json.load(f)
        return files
...
def make_force_align_pipeline(files = None):
    ...
    p = mauser.Pipeline(files, textgrid_directory, language = 'ita-IT')
    return p
