...
directory = '../../LD/COMMON_VOICE_SPANISH/' #version 4
...
def make_files(force_make = False):
    if not force_make and Path('spanish_files.json').exists():
        with open('spanish_files.json', 'r') as f:
            files = json.load(f)
        return files
...
def make_force_align_pipeline(files = None):
    ...
    p = mauser.Pipeline(files, textgrid_directory, language = 'spa-ES')
    return p
